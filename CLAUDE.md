...
- `make test` — build and run the pure-C++ unit tests (`tests/run_tests.cpp`).
  569 checks over the tokenizer, Markdown parser, terminal output stream,
  settings parser, comment toggling, and the LaTeX and SyncTeX readers
  (including the LaTeX preview's click-to-source matching).
...
- tectonic 0.17.0 lives at
  ...
  `~/Library/Caches/TectonicProject.Tectonic` (about 44 MB).
