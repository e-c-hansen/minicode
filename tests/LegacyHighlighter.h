// LegacyHighlighter.h — the whole-document lexer SyntaxHighlighter used before
// it lexed line by line (as of release 1.2.2), frozen here as a test oracle.
// The tests check that SyntaxHighlighter::highlight still produces exactly
// these tokens, so the line-at-a-time rewrite changed no colors. Do not "fix"
// anything in here; change the real lexer and the tests instead.
#pragma once
...
struct LanguageDef {
    ...
    std::string blockEnd;
    std::string stringDelims;
    bool tripleQuotes = false;
    bool preprocHash = false;
...
std::vector<Token> highlight(const std::string& text, const std::string& ext) {
