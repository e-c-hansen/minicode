...
There is a terminal you can pull up at the bottom with Control backtick, and you can drag the bar above it to resize. It runs a single persistent zsh session on a real pseudo terminal, with your own startup files loaded, so aliases and functions from your .zshrc are there and state carries across commands. Output streams in as it is printed, Control C interrupts whatever is running, and programs that ask for input get it, including password prompts, which switch the input line to hidden text. A command that fails shows its exit status underneath. It keeps your command history on the up and down arrows, and clicking anywhere in the panel drops the cursor on the input line. It shows output as a scrolling log rather than emulating a screen, so full screen programs like vim or htop are still out of reach, but for the everyday things you reach for, git, make, ls, running a script, it works the way you would want. There is also an embedded browser you can toggle with Shift Command B, which is a real WebKit view with a URL bar and back, forward, and reload buttons.
